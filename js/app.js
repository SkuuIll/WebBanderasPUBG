...
const btnCompetitiveMode= document.getElementById('btnCompetitiveMode');
...
const I18N = {
  es: {
    ...
    flag_style_rectangle: '█ Rectangulares',
    flag_style_rounded: '▢ Bordes redondeados',
    flag_style_circle: '● Circulares',
    flag_style_icon: '◆ Ícono',
    flag_style_compact: '▨ Claras en pequeño',
    flag_style_square: '■ Cuadradas Minimalistas',
    btn_competitive_mode: 'Modo rápido competitivo',
    btn_randomize: 'Aleatorias',
    btn_select_visible: 'Seleccionar visibles',
    ...
    confirm_competitive_btn: 'Generar 100',
    confirm_competitive_mode: 'El modo competitivo reemplazará la selección actual por 100 banderas únicas optimizadas para slots de torneo.',
  ...
  en: {
    ...
    flag_style_rectangle: '█ Rectangular',
    flag_style_rounded: '▢ Rounded corners',
    flag_style_circle: '● Circular',
    flag_style_icon: '◆ Icon',
    flag_style_compact: '▨ Small-size clarity',
    flag_style_square: '■ Minimal Squares',
    btn_competitive_mode: 'Competitive quick mode',
    btn_randomize: 'Random',
    ...
    confirm_competitive_btn: 'Generate 100',
    confirm_competitive_mode: 'Competitive mode will replace the current selection with 100 unique flags optimized for tournament slots.',
...
const COMPETITIVE_FLAG_ORDER = [
  'ca', 'ar', 'br', 'cl', 'mx', 'uy', 'py', 'us', 'pe', 'es',
  'co', 'pt', 'bo', 'fr', 've', 'de', 'th', 'ec', 'it', 'gb',
  'jp', 'kr', 'cn', 'au', 'tr', 'za', 'ru', 'in', 'nz', 'sa',
  'ma', 'se', 'qa', 'ng', 'fi', 'ae', 'gh', 'no', 'eg', 'cm',
  'dk', 'dz', 'sn', 'ch', 'tn', 'ci', 'be', 'pl', 'jo', 'gr',
  'nl', 'lb', 'hr', 'il', 'ro', 'id', 'rs', 'ph', 'ua', 'my',
  'cz', 'sg', 'sk', 'vn', 'hu', 'tw', 'ie', 'hk', 'is', 'mn',
  'si', 'kz', 'bg', 'uz', 'al', 'pk', 'ba', 'bd', 'me', 'lk',
  'mk', 'np', 'ge', 'af', 'am', 'cu', 'az', 'gt', 'ir', 'pa',
  'iq', 'hn', 'do', 'pr', 'cr', 'jm', 'ni', 'ke', 'at', 'sv'
];

// ─── FLAG STYLE HELPER ──────────────────────────────────────────
const FLAG_STYLE_CONFIG = {
  realistic: {
    className: 'flag-style-realistic',
    canvasShape: null,
    preferSquareSource: false,
    padding: 0,
    frame: false
  },
  rectangle: {
    className: 'flag-style-rectangle',
  ...
  circle: {
    className: 'flag-style-circle',
    canvasShape: 'circle',
    preferSquareSource: true,
    padding: 0,
    frame: true
  },
  icon: {
    className: 'flag-style-icon',
    canvasShape: 'rounded',
    preferSquareSource: true,
    padding: 0.14,
    frame: true
  ...
  square: {
    className: 'flag-style-square square-style',
    canvasShape: null,
    preferSquareSource: true,
    padding: 0,
    frame: false
...
function getFlagStyleConfig(style = currentFlagStyle) {
  return FLAG_STYLE_CONFIG[normalizeFlagStyle(style)] || FLAG_STYLE_CONFIG.realistic;
}

function getFlagUrl(country, size = 320) {
  return getFlagSources(country, size)[0];
}

function normalizeFlagStyle(style) {
  return FLAG_STYLE_CONFIG[style] ? style : 'realistic';
}

function getFlagSources(country, size = 320) {
  const iso = country.iso;
  const wideSize = Math.max(40, size);
  const compactSize = Math.max(80, Math.round(size / 2));
  const flagcdnWide = `https://flagcdn.com/w${wideSize}/${iso}.png`;
  const flagcdnCompact = `https://flagcdn.com/w${compactSize}/${iso}.png`;
  const squareSvg = `https://kapowaz.github.io/square-flags/flags/${iso}.svg`;
  const config = getFlagStyleConfig();
  const sources = config.preferSquareSource
    ? [squareSvg, flagcdnWide, flagcdnCompact]
    : [flagcdnWide, squareSvg, flagcdnCompact];
  return [...new Set(sources)];
}

function getFlagImgClass(extra = '') {
  return ['flag-img', getFlagStyleConfig().className, extra].filter(Boolean).join(' ');
}

function getFlagFallbackAttr(country, size = 80) {
  const [, ...fallbacks] = getFlagSources(country, size);
  return escapeHtml(JSON.stringify(fallbacks));
}

function renderFlagImage(item, size = 80, extraClass = '') {
...
function getPlatformLogoUrl(platform) {
  if (!platform) return { primary: '', fallback: '' };
  const tag = platform.tag || '';
  const slug = PLATFORM_LOGO_MAP[tag] || tag;
...
async function ensurePlatformSupport() {
  ...
  platformSupportPromise = (async () => {
    ...
    results.sort((a, b) => platformsDB.indexOf(a) - platformsDB.indexOf(b));
...
function applyFlagStyleToCanvas(context, img, S, item) {
  ...
  drawFlagContain(context, img, S, item, {
    shape: config.canvasShape,
    padding: config.padding,
    forceContain: currentFlagStyle === 'rectangle' || currentFlagStyle === 'icon' || currentFlagStyle === 'compact'
  });

  if (config.frame) {
    drawFlagFrame(context, S, config.canvasShape || currentShape, config.padding);
  }
}
...
function enableMobileLayout() {
  ...
  document.body.classList.remove('is-tablet-layout', 'is-desktop-layout');
}

function enableTabletLayout() {
  document.body.classList.add('is-tablet-layout');
  document.body.classList.remove('is-mobile-layout', 'is-desktop-layout');
}

function enableDesktopLayout() {
  document.body.classList.add('is-desktop-layout');
  document.body.classList.remove('is-mobile-layout', 'is-tablet-layout');
}

function enableTouchGestures() {
  if (canvas && !touchGesturesEnabled) {
...
function applyFlagStyleSetting(style) {
  ...
  if (flagStyleSelect) flagStyleSelect.value = normalized;
}
...
function init() {
  ...
  try {
    ...
    if (raw) {
      ...
      if (selectedSlots.length > 0 && currentPreviewIdx === -1) {
        currentPreviewIdx = 0;
      }
    }
  } catch(e) {}
  ...
  if (btnCompetitiveMode) {
    btnCompetitiveMode.addEventListener('click', () => {
      if (selectedSlots.length) {
        openConfirmModal({
          message: t('confirm_competitive_mode'),
          confirmText: t('confirm_competitive_btn'),
          cancelText: t('confirm_cancel'),
          onConfirm: applyCompetitiveMode
        });
        return;
      }
      applyCompetitiveMode();
    });
  }
...
function applySettings(s) {
  ...
  if (s.opacity)       { opacityInput.value = s.opacity; opacityValue.textContent = s.opacity + '%'; }
  if (s.shadow !== undefined) {
    shadowToggle.checked = s.shadow;
    shadowOptionsRow.hidden = !shadowToggle.checked;
  }
  if (s.shadowColor)   shadowColor.value     = s.shadowColor;
...
function saveRoster() {
  ...
  downloadBlob(blob, `FlagForge_Roster_${new Date().toISOString().slice(0,10)}.json`);
...
function getSorted(arr) {
  ...
  return [...arr].sort((a, b) => {
    ...
    if (sortMode === 'iso')        return (a.iso || a.tag || '').localeCompare(b.iso || b.tag || '');
    return 0;
  });
}
...
function renderRoster() {
  ...
  selectedSlots.forEach((c, i) => {
    ...
    div.addEventListener('dragover', e => {
      ...
      div.classList.add(isAbove ? 'drag-above' : 'drag-below');
    });
    ...
    div.addEventListener('drop', e => {
      ...
      const rect = div.getBoundingClientRect();
      let insertIdx = (e.clientY - rect.top) < (rect.height / 2) ? targetIdx : targetIdx + 1;
      if (dragSrcIdx < insertIdx) insertIdx--;
      if (dragSrcIdx === insertIdx) return;
      pushUndo();
      const moved = selectedSlots.splice(dragSrcIdx, 1)[0];
      selectedSlots.splice(insertIdx, 0, moved);
      currentPreviewIdx = insertIdx;
      updateUI();
      dragSrcIdx = null;
    });
...
function drawFlagContain(context, img, S, item, options = {}) {
  const padding = Math.max(0, Math.min(0.3, options.padding || 0));
  ...
  let dw, dh, dx, dy;
  if (squareFlag.checked || options.forceContain) {
    ...
    dw = box; dh = box; dx = inset; dy = inset;
  }

  // Apply shape clip
  context.save();
  applyShapeClip(context, S, options.shape, inset);
  context.drawImage(img, dx, dy, dw, dh);
  context.restore();
}

function applyShapeClip(context, S, shapeOverride, inset = 0) {
  const shape = shapeOverride || currentShape;
  const box = S - inset * 2;
  context.beginPath();
  if (shape === 'circle') {
    context.arc(S / 2, S / 2, box / 2, 0, Math.PI * 2);
  } else if (shape === 'rounded') {
    const r = box * 0.12;
    const x = inset;
    const y = inset;
    context.moveTo(x + r, y);
    context.lineTo(x + box - r, y);
    context.quadraticCurveTo(x + box, y, x + box, y + r);
    context.lineTo(x + box, y + box - r);
    context.quadraticCurveTo(x + box, y + box, x + box - r, y + box);
    context.lineTo(x + r, y + box);
    context.quadraticCurveTo(x, y + box, x, y + box - r);
    context.lineTo(x, y + r);
    context.quadraticCurveTo(x, y, x + r, y);
  } else {
    context.rect(inset, inset, box, box);
  }
  context.clip();
}

function drawFlagFrame(context, S, shape, padding = 0) {
  const inset = Math.max(1, S * padding + 1);
  const box = S - inset * 2;
  context.save();
  ...
  if (shape === 'circle') {
    ...
    context.lineTo(inset + box - r, inset);
    context.quadraticCurveTo(inset + box, inset, inset + box, inset + r);
    context.lineTo(inset + box, inset + box - r);
    context.quadraticCurveTo(inset + box, inset + box, inset + box - r, inset + box);
    context.lineTo(inset + r, inset + box);
    context.quadraticCurveTo(inset, inset + box, inset, inset + box - r);
...
function updatePreview(country, number) {
  ...
  if (currentMode === 'platforms') {
    ...
    fallbackSources = logo.fallback ? [logo.fallback] : [];
  } else {
    const sources = getFlagSources(country, 320);
    img.src = sources[0];
...
function exportCSVOnly() {
  if (!selectedSlots.length) return;
  const rows = [formatCsvRow(['TeamNumber', 'TeamName', 'TeamShortName', 'ImageFileName', 'TeamColor'])];
  selectedSlots.forEach((c, i) => {
    const num  = i + getStartNum();
  ...
  downloadBlob(blob, 'TeamInfo.csv');
...
async function generatePack() {
  ...
  try {
    ...
    const csvRows      = [formatCsvRow(['TeamNumber', 'TeamName', 'TeamShortName', 'ImageFileName', 'TeamColor'])];
...
function fetchImageAndDraw(item, number, can, ctx2d, S) {
  return new Promise(resolve => {
    ...
    if (currentMode === 'platforms') {
      ...
      const sources = getFlagSources(item, 320);
      img.src = sources[0];
      fallbackSources = sources.slice(1);
    }
...
function getCompetitiveFlags() {
  const byIso = new Map(db.map(item => [item.iso, item]));
  return COMPETITIVE_FLAG_ORDER.map(iso => byIso.get(iso)).filter(Boolean);
}

function applyCompetitiveMode() {
  const flags = getCompetitiveFlags();
  pushUndo();
  if (currentMode !== 'flags') {
    setMode('flags', { skipConfirm: true, skipUndo: true, silent: true });
  }
  selectedSlots = flags;
  currentPreviewIdx = selectedSlots.length ? 0 : -1;
  numberStart.value = 1;
  applyFlagStyleSetting('compact');
  updateUI();
  showToast(t('toast_competitive_ready', { count: selectedSlots.length }), 'success');
}
